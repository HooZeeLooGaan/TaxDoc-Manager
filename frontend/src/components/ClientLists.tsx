...
export function ClientsList({
  ...
  const handleClientDelete = async (e: React.MouseEvent, clientId: string) => {
    e.preventDefault();
  ...
  if (loading) {
    return (
      <div className="p-6 text-xs text-text-muted animate-pulse">
        Loading roster...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="m-4 p-3 bg-rose-50 text-xs text-rose-700 rounded-lg">
        {error}
      </div>
    );
  }
  ...
  if (clients.length === 0) {
    return (
      <div className="p-8 text-xs text-text-muted text-center">
