...
import RequirementFormOverlay from './RequirementFormOverlay';
...
export default function ClientRequirements({
  ...
  const [isAddRequirementOverlayEnabled, setIsAddRequirementOverlayEnabled] =
  ...
  const [activeReqForUpload, setActiveReqForUpload] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    setIsAddRequirementOverlayEnabled(false);
  ...
  const handleWaiveItem = async (reqId: string) => {
    setLoading(true);
    setError(null);
    ...
    try {
      await updateRequirements(reqId, payload);
      handleRederiveRequirements?.();
    } catch (err: any) {
      setError(
        err?.detail || err?.message || 'Failed to update requirement status'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleRederiveRequirements = async () => {
    ...
    try {
      ...
      alert(
        `Error rederiving requirements: ${err?.message || 'Something went wrong'}`
      );
    } finally {
  ...
  const progressPercent =
    totalReqs > 0 ? Math.round((fulfilledReqs / totalReqs) * 100) : 0;

  if (selectedDocId) {
    return (
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-sm h-full flex flex-col space-y-3">
        {/* Header Container */}
        <div className="flex items-center justify-between pb-3 border-b border-slate-200">
          ...
          <button
            ...
            className="p-1 text-slate-400 hover:text-slate-700 rounded-md hover:bg-slate-200 transition-colors cursor-pointer text-sm leading-none flex items-center justify-center w-7 h-7"
            title="Close Panel"
          >
            ✕
          </button>
        ...
        {/* Document Preview taking up remaining space */}
  ...
  return (
    <>
      ...
      {!loading && !error && (
        <aside className="relative w-full h-9/10 flex flex-col bg-white border-l border-slate-300 font-sans text-xs text-slate-800 overflow-hidden shadow-2xl">
          {isAddRequirementOverlayEnabled && (
            <div>
              <RequirementFormOverlay
                clientId={client.id}
                clientName={client.primary_name}
                isOpen={isAddRequirementOverlayEnabled}
                onSuccess={handleRederiveRequirements}
                onClose={() => setIsAddRequirementOverlayEnabled(false)}
              />
            </div>
          )}
                ...
                {documents.map((file) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      <button
                        onClick={() => handleDocumentPreview(file.file_id)}
            ...
            <section className="bg-white border border-slate-300 rounded-md overflow-hidden shadow-xs">
              ...
              <div className="bg-slate-100 border-b border-slate-300 px-3 py-2 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      setIsAddRequirementOverlayEnabled(true);
                    }}
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {(!requirements || requirements.length === 0) && (
                      <tr>
                        ...
                        >
                          No requirement documents specified for the client
                        </td>
                      </tr>
                    )}
